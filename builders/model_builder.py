...
from models.conv_test_net import build_test_net
...
SUPPORTED_MODELS = ["conv_test_net", "FC-DenseNet56", "FC-DenseNet67", "FC-DenseNet103", "Encoder-Decoder", "Encoder-Decoder-Skip", "RefineNet",
    "FRRN-A", "FRRN-B", "MobileUNet", "MobileUNet-Skip", "PSPNet", "GCN", "DeepLabV3", "DeepLabV3_plus", "AdapNet",
    "DenseASPP", "DDSC", "BiSeNet", "custom"]

SUPPORTED_FRONTENDS = ["None", "ResNet50", "ResNet101", "ResNet152", "MobileNetV2", "InceptionV4"]
...
def build_model(model_name, net_input, num_classes, crop_width, crop_height, frontend="ResNet101", is_training=True):
    ...
    if model_name == "conv_test_net":
        network = build_test_net(net_input, num_classes, 'lrf_conv2d', False)
    elif model_name == "FC-DenseNet56" or model_name == "FC-DenseNet67" or model_name == "FC-DenseNet103":
